...
#line 45 "parser.y"
    { 
		...
		new_field->order = (yyvsp[(1) - (4)].intval);
		new_field->size = (yyvsp[(2) - (4)].intval);
		new_field->display = (yyvsp[(3) - (4)].intval);
		new_field->identifier = (yyvsp[(4) - (4)].textval);
...
/* Line 1806 of yacc.c  */
#line 86 "parser.y"
    { (yyval.textval) = strdup((yyvsp[(1) - (1)].textval)); }
    break;
...
#line 90 "parser.y"
    { 
		...
		nextheader->prefix = (yyvsp[(2) - (3)].textval);
		nextheader->fieldname = (yyvsp[(3) - (3)].textval);
		...
		for(tmp = el_list ;; tmp=tmp->next)
		{
		     ...
		     */
		    if(strcmp((yyvsp[(3) - (3)].textval), tmp->data->field->identifier) == 0)
		    {
